 ...
 * Author: gabizou
 */
public class TownReClaimCommand extends BasicTownInteractiveCommand {

    Map<String, Location> pending = new HashMap<>();

    public TownReClaimCommand(TownManager manager) {
        super(manager, "TownReclaim");
        setDescription("Marks the location of your town's center, can only be used to setup a town's region once!");
        setUsage("/town reclaim");
        setPermission("townships.town.reclaim");
    ...
    class StateA extends BasicInteractiveCommandState {
        ...
        public boolean execute(CommandSender sender, String identifier, String[] args) {
            ...
            Citizen citizen = plugin.getCitizenManager().getCitizen(player);
            ...
            if (citizen.getTown().hasRegions() && manager.getRegionManager().getRegions().containsKey(citizen.getTown())) {
                Messaging.send(sender, getMessage("town_cant_reclaim"));
                cancelInteraction(sender);
                return true;
            }

            if (!citizen.getTown().hasRegions()) { // Do NOT process if the Town has not upgraded with regions yet.
                Messaging.send(sender, getMessage("town_cant_reclaim"));
                cancelInteraction(sender);
                return true;
            }
