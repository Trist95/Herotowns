 ...
 * @author gabizou
 *
 * ChatBlock - An object that helps organize and format a message to a player when involving lists
 * and a requirement for correct column formatting is necessary. Borrowed from SimpleClans.
 *
 * TODO: Need to revamp some of the logic here
 */
public class ChatBlock {
    ...
    boolean sendBlock(CommandSender player, String prefix, int amount) {
        ...
        for (int i = 0; i < amount; i++) {
            ...
            for (int sid = 0; sid < row.length; sid++) {
                ...
                double columnSize = (columnSizes.size() >= (sid + 1)) ? columnSizes.get(sid) : 0;
                String align = (columnAlignments.size() >= (sid + 1)) ? columnAlignments.get(sid) : "left";

                if (align.equalsIgnoreCase("right")) {
                    if (msgLength(section) > columnSize) {
                        col = cropLeftToFit(section, columnSize);
                    } else if (msgLength(section) < columnSize) {
                        col = paddLeftToFit(section, columnSize);
                    }
                } else if (align.equalsIgnoreCase("left")) {
                    if (msgLength(section) > columnSize) {
                        col = cropRightToFit(section, columnSize);
                    } else if (msgLength(section) < columnSize) {
                        col = paddRightToFit(section, columnSize);
                    }
                } else if (align.equalsIgnoreCase("center")) {
                    if (msgLength(section) > columnSize) {
                        col = cropRightToFit(section, columnSize);
                    } else if (msgLength(section) < columnSize) {
                        col = centerInLineOf(section, columnSize);
                    }
                }
    ...
    /**
     * @param msg
     * @return centerInLineOf(message, length);
     */
